...
def resnet18_features(pretrained=False, **kwargs):
    ...
    """
    write_info("pretrained: ", pretrained)
...
def resnet50_features(pretrained=False, **kwargs):
    ...
    # newmodel = torch.nn.Sequential(*(list(resnext50_32x4d.children())[:-2]))
    # print(newmodel)
    # return newmodel
...
if __name__ == '__main__':
    ...
    print("r18_features")
    pytorch_total_params = sum(p.numel() for p in r18_features.parameters() if p.requires_grad)
    print(f"\033[0;30;43m{pytorch_total_params}\033[0m")
    print(r18_features)

    r34_features = resnet34_features(pretrained=True)
    print("r34_features")
    pytorch_total_params = sum(p.numel() for p in r34_features.parameters() if p.requires_grad)
    print(f"\033[0;30;43m{pytorch_total_params}\033[0m")
    print(r34_features)

    r50_features = resnet50_features(pretrained=True)
    print("r50_features")
    pytorch_total_params = sum(p.numel() for p in r50_features.parameters() if p.requires_grad)
    print(f"\033[0;30;43m{pytorch_total_params}\033[0m")
    print(r50_features)

    r101_features = resnet101_features(pretrained=True)
    print("r101_features")
    pytorch_total_params = sum(p.numel() for p in r101_features.parameters() if p.requires_grad)
    print(f"\033[0;30;43m{pytorch_total_params}\033[0m")
    print(r101_features)

    r152_features = resnet152_features(pretrained=True)
    print("r152_features")
    pytorch_total_params = sum(p.numel() for p in r152_features.parameters() if p.requires_grad)
    print(f"\033[0;30;43m{pytorch_total_params}\033[0m")
    print(r152_features)
