...
def upsample(tree: ProtoTree, project_info: dict, project_loader: DataLoader, folder_name: str, args: argparse.Namespace, save_dir:str):
    # dir = os.path.join(os.path.join(args.log_dir, args.dir_for_saving_images), folder_name)
    dir = os.path.join(save_dir, args.dir_for_saving_images, folder_name)
    # print(" ======= debug ======= upsample dir: ", dir)
    if not os.path.exists(dir):
        os.makedirs(dir)
    with torch.no_grad():
        sim_maps, project_info = get_similarity_maps(tree, project_info)
        print("\nUpsampling prototypes for visualization...")
        if 'cifar10' in args.dataset.lower():
            # print("type of project_loader: ", type(project_loader))
            imgs = list(project_loader.dataset.dataset.data)
        else:
            imgs = project_loader.dataset.dataset.img_path
        for node, j in tree._out_map.items():
            if node in tree.branches: #do not upsample when node is pruned
                ...
                if 'cifar10' in args.dataset.lower():
                    x = imgs[prototype_info['input_image_ix']]
                else:
                    x = Image.open(imgs[prototype_info['input_image_ix']])
                ...
                if 'cifar10' in args.dataset.lower():
                    plt.imsave(fname=os.path.join(dir,'%s_original_image.png'%str(decision_node_idx)), arr=x, vmin=0.0,vmax=1.0)
                else:
...
#             # print("type of project_loader: ", type(project_loader))
#             imgs = list(project_loader.dataset.data)
#         else:
