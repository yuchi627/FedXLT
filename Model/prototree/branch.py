...
class Branch(Node):
    ...
    def forward(self, xs: torch.Tensor, **kwargs):
        ...
        if not self._log_probabilities:
            ...
            l_dists, _, l_logit = self.l.forward(xs, **kwargs)  # shape: (bs, k)
            r_dists, _, r_logit = self.r.forward(xs, **kwargs)  # shape: (bs, k)
            # Weight the probability distributions by the decision node's output
            ps = ps.view(batch_size, 1)
            return (1 - ps) * l_dists + ps * r_dists, node_attr, (1 - ps) * l_logit + ps * r_logit # shape: (bs, k)
        else:
            # Store decision node probabilities as node attribute
            node_attr[self, 'ps'] = ps
            ...
            logs_stacked_logit = torch.stack((oneminusp + l_logit, ps + r_logit))
            return torch.logsumexp(logs_stacked, dim=0), node_attr, torch.logsumexp(logs_stacked_logit, dim=0)  # shape: (bs,)
